...
def get_classes(dataset):
    ...
    if is_str(dataset):
        if dataset in alias2name:
            ...
            raise ValueError(f'Unrecognized datasets: {dataset}')
    else:
        raise TypeError(f'datasets must a str, but got {type(dataset)}')
    return labels


def get_palette(dataset):
    """Get class palette (RGB) of a datasets."""
    alias2name = {}
    for name, aliases in dataset_aliases.items():
        for alias in aliases:
            alias2name[alias] = name

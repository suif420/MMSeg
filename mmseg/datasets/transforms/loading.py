...
class LoadAnnotations(MMCV_LoadAnnotations):
    """Load annotations for semantic segmentation provided by datasets.
    ...
    def __init__(
        ...
        if self.reduce_zero_label is not None:
            warnings.warn('`reduce_zero_label` will be deprecated, '
                          ...
                          'set `reduce_zero_label=True` when datasets '
                          'initialized')
    ...
    def _load_seg_map(self, results: dict) -> None:
        ...
        assert self.reduce_zero_label == results['reduce_zero_label'], \
            'Initialize datasets with `reduce_zero_label` as ' \
...
class LoadBiomedicalAnnotation(BaseTransform):
    """Load ``seg_map`` annotation provided by biomedical datasets.
